$(function () {
            ...
            const render = function () {
                ...
                $.ajax({
                    ...
                    .then(function (data) {
                        let htmlstr = '';
                        data.forEach(element => {htmlstr += `<h5 class="card-title">${element.itemName}</h5>`; htmlstr += `<h6 class="card-subtitle mb-2 text-muted">#${element.itemID} / In stock: ${element.itemCount}</h6>`; htmlstr += `<button id="edit" data-id=${element.itemID} class="btn btn-primary">Edit</button>`; htmlstr += `<div id="${element.itemID}">`; htmlstr += `</div>`; htmlstr += `<hr />`;
                    });
